let contenedorMarvel = document.getElementById('contenedorMarvel');
let contenedorDC = document.getElementById('contenedorDC');

// Supongamos que tienes un array de personajes para Marvel y DC
let personajesMarvel = [  {
        ...
        "parrafo" : "orem ipsum dolor sit amet consectetur adipisicing elit. Repellendus voluptatibus temporibus quos alias illo quidem odio itaque exercitationem recusandae cumque sunt, aut commodi possimus id, voluptates officia omnis quod nesciunt.",
...
function crearPersonajes(personajes, contenedor) {
    personajes.forEach((personaje, index) => {
        let divPersonaje = document.createElement('div');
        divPersonaje.className = 'personaje';
        divPersonaje.id = 'personaje' + index;
        ...
        boton.onclick = function() { document.getElementById('modal' + contenedor.id + index).showModal(); }; // Usa el ID único del diálogo


        let dialog = document.createElement('dialog');
        dialog.id = 'modal' + contenedor.id + index;

        let divFoto3 = document.createElement('div');
        divFoto3.className = 'foto-3';
        let img3 = document.createElement('img');
        img3.src = personaje.picture;
        img3.className = 'foto-1';
        divFoto3.appendChild(img3);

        let divParrafo = document.createElement('div');
        divParrafo.className = 'parrafo';
        let p = document.createElement('p');
        p.textContent = personaje.parrafo;
        divParrafo.appendChild(p);

        let closeButton = document.createElement('button');
        closeButton.textContent = 'Cerrar';
        closeButton.onclick = function() { document.getElementById('modal' + contenedor.id + index).close(); }; // Usa el ID único del diálogo


        dialog.appendChild(divFoto3);
        dialog.appendChild(divParrafo);
        dialog.appendChild(closeButton);

        divPersonaje.appendChild(divFoto);
        divPersonaje.appendChild(h2);
        divPersonaje.appendChild(boton);
        divPersonaje.appendChild(dialog);
...
// Crear personajes para Marvel y DC
